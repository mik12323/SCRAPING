...
-- 0.5 Create listing_views table (for click tracking)
...
-- 1. Create listings table (UPDATED with color and mileage)
...
-- Add color and mileage columns if upgrading old DB
...
-- 1.5 Add edit_proposal column for tracking edits (NEW)
ALTER TABLE public.listings ADD COLUMN IF NOT EXISTS edit_proposal JSONB DEFAULT NULL;
...
-- 4. RLS Policies (FIXED - drop first if exists)
...
-- 6. Create storage bucket for car images
...
-- OR run this SQL (may require admin rights):
...
-- 3. Click "Run"
-- 4. Then go to Storage > Create Bucket if not auto-created:
--    - Name: car-images
--    - Public: YES
--    - File size limit: 5MB
--    - Allowed MIME types: image/jpeg, image/png, image/webp
