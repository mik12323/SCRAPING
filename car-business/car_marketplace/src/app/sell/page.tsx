...
import { CAR_BRANDS } from '@/lib/types';
...
export default function SellCarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const baseSlug = `${formData.brand.toLowerCase()}-${formData.model.toLowerCase().replace(/\s+/g, '-')}-${formData.year}-${formData.price.replace(/,/g, '')}`;
      ...
      if (newListing?.id) {
        ...
        const properSlug = generateSlug(formData.brand, formData.model, formData.year, parseInt(formData.price.replace(/,/g, '')), shortId, color);
      ...
      // Notify admins about new pending listing (skip if admin posted)
      if (!isAdmin && newListing?.id) {
        await fetch('/api/notify-admins', {
          ...
          body: JSON.stringify({
            ...
            message: `${formData.brand} ${formData.model} (${formData.year}) - ₱${formData.price}`,
          }),
        });
      }
