...
import { BODY_TYPES, CAR_BRANDS } from '@/lib/types';
...
export default function EditCarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (car) {
      ...
      setFormData({
        ...
        originalPrice: car.original_price ? car.original_price.toLocaleString() : '',
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'price' || name === 'originalPrice') {
      ...
      if (!isNaN(num)) {
        setFormData(prev => ({ ...prev, [name]: num.toLocaleString() }));
      }
    } else {
    ...
    try {
      ...
      if (car) {
        const fieldMappings = [
          ...
          { formKey: 'price', carKey: 'price', dbKey: 'price', transform: (v: any) => parseInt(v.replace(/,/g, '')) },
          { formKey: 'originalPrice', carKey: 'original_price', dbKey: 'original_price', transform: (v: any) => v ? parseInt(v.replace(/,/g, '')) : null },
      ...
      if (shouldRegenerateSlug(car, formData)) {
        const baseSlug = generateSlug(formData.brand, formData.model, formData.year, parseInt(formData.price.replace(/,/g, '')), shortId, color);
      ...
      const updateData: any = {
        ...
        original_price: formData.originalPrice ? parseInt(formData.originalPrice.replace(/,/g, '')) : null,
      ...
      if (updateError) {
        ...
        // Notify admins about pending edit review (skip if admin edited)
        if (!isAdmin && car) {
          await fetch('/api/notify-admins', {
            ...
            body: JSON.stringify({
              ...
              message: `${formData.brand} ${formData.model} (${formData.year}) has pending edit changes.`,
            }),
          });
        }
